...
def postprocessing(df):
    nlp_df = load_dataset(filename="./data/test_with_nlp_prediction.csv")
    nlp_df = nlp_df[nlp_df.rentFromNLP > 100]
    nlp_df = nlp_df[nlp_df.rentFromNLP < 6000]
    print(nlp_df.head())
    for i in range(len(nlp_df)):
        df.loc[df["id"] == nlp_df["id"][i], "rent"] = nlp_df["rentFromNLP"][i]
    return df
...
def main():
    ...
    model = XGBRegressor(
        tree_method="hist", objective="reg:squarederror", enable_categorical=True
    )
...
def test():
    df = load_dataset("./new_submission.csv")
    postprocessing(df)
    df.to_csv("./new_submission_2.csv")
